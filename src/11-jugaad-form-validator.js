...
export function validateForm(formData) {
  ...
  if (typeof phone !== 'string' || phone.length !== 10 || !["6", "7", "8", "9"].includes(phone[0]) || !/^\d{10}$/.test(phone)) {
    errors.phone = "Invalid Indian phone number";
  }
  
  const ageVal = formData?.age;
  const age = typeof ageVal === 'string' ? parseInt(ageVal, 10) : ageVal;
  if (typeof age !== 'number' || isNaN(age) || !Number.isInteger(age) || age < 16 || age > 100) {
    errors.age = "Age must be an integer between 16 and 100";
  }
  
  const pincode = formData?.pincode;
  if (typeof pincode !== 'string' || pincode.length !== 6 || pincode[0] === "0" || !/^\d{6}$/.test(pincode)) {
